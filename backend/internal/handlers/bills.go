...
func (h *BillHandler) serveArtifact(c *gin.Context, inline bool) {
	...
	if inline && shouldDecorateMarketplaceEmailPreview(art, contentType) {
		data = []byte(decorateMarketplaceEmailPreviewHTML(string(data)))
	}
...
func shouldDecorateMarketplaceEmailPreview(art *models.BillArtifact, contentType string) bool {
	if art == nil {
		return false
	}
	...
	filename := strings.ToLower(strings.TrimSpace(art.Filename))
	return kind == "email_html" || strings.HasSuffix(filename, ".html")
}
...
func styleAllHTMLTags(input, tagPrefix, style, dataAttr string) string {
	...
	for {
		idx := strings.Index(lower[pos:], lowerPrefix)
		if idx < 0 {
